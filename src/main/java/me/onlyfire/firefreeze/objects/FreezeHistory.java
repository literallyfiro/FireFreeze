...
public class FreezeHistory {
    ...
    public void sendTo(CommandSender sender) {
        if (!(staff.isEmpty() || time.isEmpty())) {
            ...
            sender.sendMessage(ColorUtil.colorize(Firefreeze.getInstance().getMessagesFile().getString("freeze_history.has_history")
            ...
            while (i < staff.size() && i < time.size()) {
                sender.sendMessage(ColorUtil.colorize(Firefreeze.getInstance().getMessagesFile().getString("freeze_history.history")
            ...
            sender.sendMessage(ColorUtil.colorize(Firefreeze.getInstance().getMessagesFile().getString("freeze_history.no_history")
                    .replace("{PLAYER}", player)));
        }
    }
}
